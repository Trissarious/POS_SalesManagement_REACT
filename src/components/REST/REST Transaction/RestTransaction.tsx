...
export const RestTransaction = ():[ (transactionid:number)=> void,(transactionid:number)=>void,(transaction:Transaction)=>void,(transaction:Transaction)=>void, Transaction|undefined, string] => {
    ...
    function addTransaction(transaction:Transaction){
        axios.post("https://dilven-springboot.onrender.com/transaction/postTransaction",{
    ...
    function editTransaction(transaction:Transaction){
        axios.put("https://dilven-springboot.onrender.com/transaction/putTransaction?transactionid=" + transaction.transactionid,{
            total_quantity: transaction.total_quantity,
            total_price: transaction.total_price,
            tendered_bill: transaction.tendered_bill,
            balance: transaction.balance,
...
function getTransactionByid (transactionid:number|undefined){
    axios.get("https://dilven-springboot.onrender.com/transaction/getByTransactionid?transactionid=" + transactionid,{
}).then((response) => {
    setTransaction(response.data);
    console.log(response.data);
})
...
function deleteByIDTransaction (transactionid:number|undefined){
    axios.delete("https://dilven-springboot.onrender.com/transaction/deleteTransaction/" + transactionid,{
    }).then((response) => {
    setTransaction(response.data);
    console.log(response.data);    
    })
